...
if opt.name == 'demo':
	save_path = os.path.join(ROOT_DIR + '/generation/results/Lab', opt.name)
else:
...
if reencode:
	for i, data in enumerate(dataset):
		...
		if simple_encode:
			feat = model.simple_encode_features(data['image'], data['label'])
			...
			feat = model.encode_features(data['image'], data['label'])
			inst_np = data['label'][0].cpu().numpy().astype(int)
			for label in np.unique(inst_np):
