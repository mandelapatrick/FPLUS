...
ROOT_DIR='/Users/mandelapatrick/Downloads/FashionPlusLocal'
...
def set_dataset_parameters(classname):
    ...
    if classname == 'humanparsing':
        with open(f'{ROOT_DIR}/separate_vae/datasets/%s/garment_label_part_map.json' % classname, 'r') as readfile:
...
def manually_update_feature(opt_feat, input_feature, swapped_partID):
    ...
    if argopt.update_full:
        ...
        opt_feat = torch.from_numpy(input_feature.feature).float() #.cuda().float()
...
def compute_updated_feature(input_feature):
    ...
    mask = torch.from_numpy(get_mask(input_feature.feature)).byte() #.cuda().byte()
    opt_feat = torch.from_numpy(input_feature.feature).float() #.cuda().float()
    opt_feat.requires_grad = True
...
def generate_segmentation_mask_per_iteration(shape_feature, updated_feature):
    ...
    '''
    # Add generator path
    parentPath = os.path.abspath(f'{ROOT_DIR}/separate_vae')
...
# Add classifier path
parentPath = os.path.abspath(f'{ROOT_DIR}/classification')
if parentPath not in sys.path:
